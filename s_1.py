...
def crop_black_borders(
    ...
    output_path,
    threshold=10,
    corner_cleanup=False,
    trim_top_pixels=0,
    trim_bottom_pixels=0,
):
    """
    ...
    with Image.open(image_path) as img:
        ...
        vertical_trim = {'top': 0, 'bottom': 0}
        if trim_top_pixels > 0 or trim_bottom_pixels > 0:
            safe_trim_top = max(0, min(trim_top_pixels, max(0, cropped.height - 1)))
            safe_trim_bottom = max(
                0,
                min(trim_bottom_pixels, max(0, cropped.height - safe_trim_top - 1)),
            )
            if safe_trim_top > 0 or safe_trim_bottom > 0:
                cropped = cropped.crop(
                    (0, safe_trim_top, cropped.width, cropped.height - safe_trim_bottom)
                )
                vertical_trim = {'top': safe_trim_top, 'bottom': safe_trim_bottom}
...
def process_folder(
    ...
    if trim_top_pixels > 0 or trim_bottom_pixels > 0:
        print(
            f"Vertical trim (pixels): top={trim_top_pixels}, bottom={trim_bottom_pixels}"
        )
    ...
    for path in files:
        ...
        result = crop_black_borders(
            ...
            trim_top_pixels=trim_top_pixels,
            trim_bottom_pixels=trim_bottom_pixels,
        )
        ...
        print(
            ...
            f"vertical_trim:{result['vertical_trim']} | "
...
def process_eye_dataset(
    dataset_dir,
    output_dir,
    threshold=10,
    trim_top_pixels_a=0,
    trim_bottom_pixels_a=0,
):
    ...
    for folder_name in subfolders:
        ...
        apply_vertical_trim = folder_name in ['train_A', 'test_A']
        process_folder(
            input_folder,
            output_folder,
            threshold=threshold,
            corner_cleanup=apply_corner_cleanup,
            trim_top_pixels=trim_top_pixels_a if apply_vertical_trim else 0,
            trim_bottom_pixels=trim_bottom_pixels_a if apply_vertical_trim else 0,
        )
...
if __name__ == '__main__':
    ...
    eye_output_dir = base_dir / 'datasets' / 'eye_cropped_1'

    process_eye_dataset(
        eye_dataset_dir,
        eye_output_dir,
        threshold=10,
        trim_top_pixels_a=110,
        trim_bottom_pixels_a=115,
    )
